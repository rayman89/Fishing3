...
namespace Fishing3.Items
{
	public class CaveCrate : ModItem
	{
		...
		{
			Tooltip.SetDefault("Hard to open.");
			DisplayName.SetDefault("Cave Crate");
		}
		...
		public override void RightClick(Player player)
		{
			...
			if (Main.rand.Next(0, 4) == 0) 		{	
				player.QuickSpawnItem(ItemID.SpiderFang, Main.rand.Next(1, 3)); }	
			}
				...
				if (Main.rand.Next(0, 12) == 0) 		{		
					int[] items = new int[] { ItemID.Rally, ItemID.MagicMirror, ItemID.HermesBoots,  ItemID.ShoeSpikes };
					int randomItem = Main.rand.Next(4);
					player.QuickSpawnItem(items[randomItem]);
													}
													
						if (Main.rand.Next(0, 14) == 0) 		{								
							int[] items = new int[] { ItemID.CloudinaBottle, ItemID.EnchantedBoomerang, ItemID.BandofRegeneration };
